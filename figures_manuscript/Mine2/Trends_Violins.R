# ========================================================
# UPDATED: Trends_Violins.R
# ========================================================

source(file.path("code", "paths+packages.R"))
library(tidyverse)
library(patchwork)
library(colorspace) # Needed to darken the borders of the violins

# 1. DEFINE YOUR METRICS AND LABELS
metrics <- c("Zero_Flow_Days_sum", "mean_dry_down_days", "hydro_doy")

labs_metrics <- c("Zero_Flow_Days_sum" = "(a) Annual No-Flow Days", 
...
gage_mean <- readr::read_csv(file.path(data_dir, "Physiographic_data.csv"), show_col_types = FALSE) %>%
  mutate(gauge_id = as.character(gauge_id)) %>%
  # Make sure we use your exact latitude/longitude column names!
...
# ========================================================
# PLOT 1: REGIONAL VIOLINS
# ========================================================

p_region <- gage_hydro_trends %>% 
  # Drop NAs so the plot doesn't render an empty "NA" region category
  ...
  # Wrap long region names so they don't overlap on the x-axis
...
ggsave(file.path("figures_manuscript/Mine2", "Trends_Violins-Region.png"),
       plot = p_region, width = 120, height = 240, units = "mm")

# ========================================================
# PLOT 2: LATITUDE VIOLINS (ADJUSTED FOR INDIA)
# ========================================================
...
gage_hydro_trends$lat_bin <- cut(gage_hydro_trends$dec_lat_va, 
                                 breaks = seq(8, 28, 4),
                                 labels = seq(10, 26, 4)) # Labels are the midpoints
...
p_lat <- gage_hydro_trends %>%
  ...
  facet_wrap(~metric, ncol = 1, dir = "v",
             scales = "free_y", labeller = as_labeller(labs_metrics)) +
  coord_flip() + # Puts latitude on the Y axis, mimicking a map!
  scale_y_continuous(name = "Trend (Kendall \u03c4)", limits = c(-tau_abs, tau_abs),
                     breaks = seq(-0.6, 0.6, 0.3), expand = c(0, 0.01)) +
...
ggsave(file.path("figures_manuscript/Mine2", "Trends_Violins-Region+Lat.png"),
       plot = final_plot, width = 190, height = 180, units = "mm")

# ========================================================
# STATISTICAL OUTPUT
# ========================================================

cat("\n--- NUMBER OF GAUGES BY REGION ---\n")
print(
  gage_hydro_trends %>% 
